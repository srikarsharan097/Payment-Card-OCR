...
# 10 is threshold value, 255 is max value
...
# RETR_EXTERNAL returns only enternal contours
# CHAIN_APPROX_SIMPLE simplifies memory and computation time in our program
...
for (i, c) in enumerate(refCnts):
	...
	roi = ref[y:y + h, x:x + w]
	# resize parameters are ref img and dimensions
...
# Sobel() is to detect the edges of an image in both horizontal and vertical directions
# CV_32F is float 
gradX = cv2.Sobel(tophat, ddepth=cv2.CV_32F, dx=1, dy=0, ksize=-1)
# absolute() is to calculate absolute value of each element
...
# THRESH_OTSU uses the histogram of the image for threshold searching process.
thresh = cv2.threshold(gradX, 0, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)[1]

# apply a second closing operation to the binary image, again
# to help close gaps between credit card number regions
# morphologyEx to apply Morphological Transformation such as: Opening; Closing;
